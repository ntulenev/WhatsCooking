...
- `AI_TEAM_OVERVIEW_PROMPT.md` - compact English prompt template used by the toolbar `Copy` action for team-level analysis.
...
#### Team Pull Request Overview

The toolbar contains a `Copy` button for project-level AI analysis. It is enabled on the open and recently merged pull request tabs and disabled on the telemetry tab. The copied PR scope is resolved as follows:

- when one or more table rows are selected, only the selected PRs are copied;
- when no rows are selected, every PR currently displayed after global and column filters is copied.

The action builds a compact English prompt from `AI_TEAM_OVERVIEW_PROMPT.md`. It includes the configured Bitbucket access details and a list of PR URLs, then asks for a concise summary of what the team is working on and which PRs need attention because of impact, risks, problems, conflicts, delays, stalled work, or review gaps. On the recently merged tab, the prompt instead treats the PRs as completed work and asks for a retrospective assessment of team activity, notable risks, delays, and follow-up needs.

> **Security warning:** both AI clipboard actions include the configured `Bitbucket:AuthEmail` and `Bitbucket:AuthApiToken`. Paste their output only into AI tools and environments you trust. The instruction inside a prompt not to repeat credentials is not a security boundary. Use a scoped, revocable token and rotate it if a prompt is exposed.
