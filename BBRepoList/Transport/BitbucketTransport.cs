...
public sealed class BitbucketTransport : IBitbucketTransport
{
    ...
    public async Task<TDto?> GetAsync<TDto>(Uri url, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            try
            {
                ...
                var formattedBody = _errorResponseParser.Parse(body, resp.Content.Headers.ContentType?.MediaType);
    ...
    private readonly IBitbucketErrorResponseParser _errorResponseParser;
}
